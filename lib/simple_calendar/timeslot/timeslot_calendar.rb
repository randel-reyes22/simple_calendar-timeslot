...
module SimpleCalendar::Timeslot
  class TimeslotCalendar < SimpleCalendar::Calendar
    ...
    def event_height(event, day)
      event_start = event.send(attribute)
      event_end = event.send(end_attribute)
    
      # Adjust event start and end times to fit within the customizable hours
      event_start = [event_start, day.to_time.change(hour: start_hour)].max
      event_end = [event_end, day.to_time.change(hour: end_hour)].min
    
      return 0 if event_start >= event_end
    ...
    def event_top_distance(event, day)
      ...
      return 0 if event_start.to_date != day
    
      # Adjust event start time to fit within the customizable hours
      event_start = [event_start, day.to_time.change(hour: start_hour)].max
    
      minutes_from_start = (event_start.hour * 60 + event_start.min) - (start_hour * 60)
